...
export default function HeroPanel({
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_0_40px_rgba(255,255,255,0.03)]">
      <div className="mb-3 text-sm uppercase tracking-[0.25em] text-red-300/70">Play fast. Avoid last.</div>
      <h2 className="mb-3 text-4xl font-black uppercase leading-none sm:text-5xl">Only one player loses.</h2>
      <p className="max-w-2xl text-base text-white/70 sm:text-lg">
        Three players enter. Full ties replay automatically. Tied winners trigger a winners round.
        Tied losers trigger a losers round. This is a play-money prototype.
      </p>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4 text-sm text-white/70">
          Buy-in: <span className="font-bold text-white">${buyIn}</span> · House cut:{" "}
          <span className="font-bold text-white">${houseCut.toFixed(2)}</span>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
        ...
        <div className="mt-2 text-xs text-white/45">Control autoplay here or from the completed outcome popup.</div>
      </div>
    </div>
  );
}