    ...
    <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-black p-4 shadow-[0_0_80px_rgba(255,0,0,0.12)]">
      ...
      <div className="relative mx-auto mt-4 flex max-w-5xl flex-wrap items-center justify-center gap-6">
        {players.map((player, index) => {
          ...
          const tones = [
            "border-lime-300/40 bg-lime-500/10 text-lime-200 shadow-[0_0_45px_rgba(132,255,120,0.25)]",
            "border-red-300/40 bg-red-500/10 text-red-200 shadow-[0_0_45px_rgba(255,80,80,0.25)]",
            "border-cyan-300/40 bg-cyan-500/10 text-cyan-200 shadow-[0_0_45px_rgba(34,211,238,0.25)]",
            "border-amber-300/40 bg-amber-500/10 text-amber-200 shadow-[0_0_45px_rgba(251,191,36,0.25)]",
          ];

          const activeTone = "border-white/20 bg-white/5 text-white shadow-[0_0_30px_rgba(255,255,255,0.08)]";

          const resultTone =
            isResults && placement === 1
              ? "ring-2 ring-lime-300/60"
              : isResults && placement === 3
              ? "opacity-75 ring-2 ring-red-400/50"
              : "";

          return (
            <div
              key={player.id}
              className={`w-[180px] rounded-3xl border p-4 text-center transition-all duration-500 ${
                resultTone
              } ${isReveal && active ? "scale-105" : "scale-100"}`}
              style={{
